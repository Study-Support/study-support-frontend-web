<template>
  <div class="full">
    <h5>Nhóm học</h5>
    <button class="mt-2" @click="choose === 1 ? (choose = 0) : (choose = 1)">
      Bạn là thành viên
    </button>
    ...
    <button class="mt-2" @click="choose === 2 ? (choose = 0) : (choose = 2)">
      Bạn là người hướng dẫn
    </button>
    <div class="group2" :class="`s${choose}`">
      <button
        v-for="group in groupsIsMentor"
        :key="group.id"
        class="d-block"
        @click="navigateTo(`/groups/${group.id}`)"
      >
    ...
    <button class="mt-2" @click="choose === 3 ? (choose = 0) : (choose = 3)">
      Tạo nhóm học
    </button>
    <div class="group3" :class="`s${choose}`">
      <button
        v-for="group in groupsIsRequireMentor"
        :key="group.id"
        class="d-block"
        @click="navigateTo(`/groups/create/${group.id}`)"
      >
        <BIconPeopleFill class="me-2" />
    ...
    <button class="mt-2" @click="choose === 4 ? (choose = 0) : (choose = 4)">
      Bạn là người hướng dẫn
    </button>
    <div class="group4" :class="`s${choose}`">
      <button
        v-for="group in groupsIsRequireGroup"
        :key="group.id"
        class="d-block"
        @click="navigateTo(`/groups/${group.id}/register-is-mentor`)"
      >
        <BIconPeopleFill class="me-2" />
        <span style="background-color: transparent" class="subject">
          {{ group.subject }}
