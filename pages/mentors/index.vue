            ...
            <BRow class="">
              <BCol>
                <div class="input-group search">
                  <input v-model="filter.a.search" class="form-control border-end-0 border" type="search"
    ...
    <BContainer>
      <div class="full-filter">
        ...
        <BRow class="filter">
          ...
          <BCol class="col-4">
            <select v-model="filter.a.subject_id" class="form-select col" required>
          ...
          <BCol class="text-end col-auto">
            <SubmitButton class="submit-button" :isDisabled="isDisabledButton" :content="'Tìm kiếm'"
              :color="'rgb(63 88 120)'" @click.prevent="submit" />
          </BCol>
        </BRow>
      </div>
      <BRow class="result pb-5">
        <BCol v-for="mentor in MentorsResult" :key="mentor.id" class="col-6 col-md-3 mt-4">
...
const filter = ref(
  {
    a: {
      ...
      subject_id: route.query.subject_id === undefined ? '' : route.query.subject_id,
...
getFilterMentorsResponse(() => {
  ...
  if (dataGetFilterMentors.value.data.data.length !== 0) {
    MentorsResult.value = MentorsResult.value.concat(dataGetFilterMentors.value.data.data);
  }
  if (dataGetFilterMentors.value.data.data.length < getConfig('constants.pagination')) {
    loading.value = false;
  }
})
